...
CFE_Status_t EXAMPLE_APP_SendHkCmd(const EXAMPLE_APP_SendHkCmd_t *Msg)
{
    ...
    */
    EXAMPLE_APP_Data.HkTlm.Payload.CommandErrorCounter = EXAMPLE_APP_Data.ErrCounter;
    EXAMPLE_APP_Data.HkTlm.Payload.CommandCounter      = EXAMPLE_APP_Data.CmdCounter;

    /*
    ** Send housekeeping telemetry packet...
    */
    CFE_SB_TimeStampMsg(CFE_MSG_PTR(EXAMPLE_APP_Data.HkTlm.TelemetryHeader));
    CFE_SB_TransmitMsg(CFE_MSG_PTR(EXAMPLE_APP_Data.HkTlm.TelemetryHeader), true);

    /*
    ** Manage any pending table loads, validations, etc.
    */
    for (i = 0; i < EXAMPLE_APP_PLATFORM_NUMBER_OF_TABLES; i++)
    {
        CFE_TBL_Manage(EXAMPLE_APP_Data.TblHandles[i]);
    }
...
{
    EXAMPLE_APP_Data.CmdCounter++;

    CFE_EVS_SendEvent(EXAMPLE_APP_NOOP_INF_EID, CFE_EVS_EventType_INFORMATION, "SAMPLE: NOOP command %s",
                      EXAMPLE_APP_VERSION);
...
{
    EXAMPLE_APP_Data.CmdCounter = 0;
    EXAMPLE_APP_Data.ErrCounter = 0;

    CFE_EVS_SendEvent(EXAMPLE_APP_RESET_INF_EID, CFE_EVS_EventType_INFORMATION, "SAMPLE: RESET command");
...
CFE_Status_t EXAMPLE_APP_ProcessCmd(const EXAMPLE_APP_ProcessCmd_t *Msg)
{
    CFE_Status_t               Status;
    void *                     TblAddr;
    EXAMPLE_APP_ExampleTable_t *TblPtr;
    const char *               TableName = "EXAMPLE_APP.ExampleTable";

    /* Sample Use of Example Table */
    EXAMPLE_APP_Data.CmdCounter++;
    Status = CFE_TBL_GetAddress(&TblAddr, EXAMPLE_APP_Data.TblHandles[0]);
    ...
    else
    {
        ...
        EXAMPLE_APP_GetCrc(TableName);

        Status = CFE_TBL_ReleaseAddress(EXAMPLE_APP_Data.TblHandles[0]);
        if (Status != CFE_SUCCESS)
        {
        ...
        {
            /* Invoke a function provided by EXAMPLE_APP_LIB */
            SAMPLE_LIB_Function();
        }
    }
...
{
    EXAMPLE_APP_Data.CmdCounter++;
    CFE_EVS_SendEvent(EXAMPLE_APP_VALUE_INF_EID, CFE_EVS_EventType_INFORMATION,
                      "EXAMPLE_APP: ValU32=%lu, ValI16=%d, ValStr=%s", (unsigned long)Msg->Payload.ValU32,
